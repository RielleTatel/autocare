...
- [ ] **Step 2: Implement certificates module + PDF processor** (BullMQ `certificates.generatePdf`: renders the public page route to PDF via headless Chromium, stores to Supabase Storage, sets `pdfUrl`; job test asserts a non-empty PDF with the score string present).
