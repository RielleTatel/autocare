...
describe("AccountScreen (M-28/M-29)", () => {
  ...
  // Announcements moved to the Home masthead (see HomeScreen.test.tsx) — the
  // feed is checked on arrival, so it must not sit behind the Account tab.
  it("no longer carries an announcements row", () => {
    render(<AccountScreen {...props} />);
    expect(screen.queryByTestId("row-announcements")).toBeNull();
  });
});
